...
#define DEFAULT_MAX_SCAN_RES                  10//8  最大扫描从机个数， 最大为8， 更大的话没验证过
...
// Default passcode
#define DEFAULT_PASSCODE                      123456 // 密码，  例程没用到
...
//static uint8 IRK[KEYLEN] = {0xE0,0xF1,0xD2,0x13,0x24,0x35,0x46,0x57,0x68,0x79,0x8a,0x9b,0XAc,0XBd,0XCe,0XDf}; 
...
//static gapDevRec_t simpleBLEDevRssi[DEFAULT_MAX_SCAN_RES];// RSSI VALUE
// Scanning state
...
static void simpleBLEAddDeviceInfo( uint8 *pAddr, uint8 addrType, uint8 Rssi );
...
void MyResolvePrivateAddr(uint8 *i, uint8 *p);
...
void SimpleBLECentral_Init( uint8 task_id )
{
  ...
  NPI_PrintValue("SMInit returned: ", SM_InitiatorInit(), 16);
...
// 按键处理
static void simpleBLECentral_HandleKeys( uint8 shift, uint8 keys )                     //Key Event
{
  ...
  if ( keys & HAL_KEY_LEFT )
  {
    ...
    if ( !simpleBLEScanning && simpleBLEScanRes > 0 )
    {
        ...
        LCD_WRITE_STRING( bdAddr2Str( simpleBLEDevList[simpleBLEScanIdx].addr ),
                          HAL_LCD_LINE_2 );
        LCD_WRITE_STRING_VALUE( "RSSI:", simpleBLEDevList[simpleBLEScanIdx].rssi, 10, HAL_LCD_LINE_7 );
  ...
  if ( keys & HAL_KEY_RIGHT )
  {
    ...
    TxPower = (TxPower+1)%4;
    HCI_EXT_SetTxPowerCmd(TxPower+1%4);
    LCD_WRITE_STRING_VALUE( "TxPower:", TxPower, 10, HAL_LCD_LINE_4 );
  }
  
  if ( keys & HAL_KEY_CENTER )  //中间按键
  {
#if ALL_PER// 连续自动连接已扫描到的从机
  ...
  {    
    NPI_PrintString("  [KEY_DOWN pressed!]\r\n");
...
void MyResolvePrivateAddr(uint8 *i, uint8 *p){
  ...
  //sspAesEncryptBasic(i,rr);
  SM_Encrypt(i,rr,temp2);
  NPI_PrintString(bdAddr2Str(i));
  NPI_PrintString(bdAddr2Str(temp2));
    ...
    case GAP_DEVICE_INFO_EVENT:
      {
        ...
        if ( DEFAULT_DEV_DISC_BY_SVC_UUID == TRUE )
        {
          ...
          {
            //向设备发现列表中添加一个设备0
            simpleBLEAddDeviceInfo( pEvent->deviceInfo.addr, pEvent->deviceInfo.addrType, (uint8)(-1*pEvent->deviceInfo.rssi) );
            LCD_WRITE_STRING_VALUE( "RSSI:", (uint8) (-1*pEvent->deviceInfo.rssi), 10, HAL_LCD_LINE_7 );
            //显示所有advert设备地址
        ...
        else if(DEFAULT_DEV_DISC_BY_SVC_UUID == FALSE)
        {
          if(pEvent->deviceInfo.eventType == 4){
            simpleBLEAddDeviceInfo( pEvent->deviceInfo.addr, pEvent->deviceInfo.addrType, (uint8)(-1*pEvent->deviceInfo.rssi) );
            LCD_WRITE_STRING_VALUE( "RSSI:", (uint8) (-1*pEvent->deviceInfo.rssi), 10, HAL_LCD_LINE_7 );
            NPI_PrintValue("eventType:", pEvent->deviceInfo.eventType, 10);
          }
    ...
    case GAP_LINK_ESTABLISHED_EVENT://设备连接
      {        
        ...
        {  
          ...
          //GAPCentralRole_GetParameter(GAPCENTRALROLE_IRK, temp);
          //GAP_ResolvePrivateAddr(temp, pEvent->linkCmpl.devAddr);
          //addr = pEvent->linkCmpl.devAddr;
...
static void simpleBLECentralPairStateCB( uint16 connHandle, uint8 state, uint8 status )
{
  uint8 IRK[KEYLEN] = {0xE0,0xF1,0xD2,0x13,0x24,0x35,0x46,0x57,0x68,0x79,0x8a,0x9b,0XAc,0XBd,0XCe,0XDf};
  uint8 peerAddr[6] = {0x50,0x18,0x0a,0x51,0xa2,0xd4};
  uint8 peerAddr2[3] = {0X1E,0X00,0X67};  //0X42,0X39,0X52,0X1E,0X66,0X67
  uint8 *p = &peerAddr[3];
  //uint8 peerAddr[6] = {0xf2,0x9b,0x98,0x55,0xce,0x62};
  ...
  else if ( state == GAPBOND_PAIRING_STATE_COMPLETE )
  {
    if ( status == SUCCESS )
    {
      //NPI_PrintValue("Resolve returned: ", GAP_ResolvePrivateAddr( IRK, peerAddr ), 16);              //test
      LCD_WRITE_STRING( "Pairing success", HAL_LCD_LINE_1 );
      //MyResolvePrivateAddr(IRK,peerAddr2);
      NPI_PrintValue("Resolve returned: ", SM_ResolveRandomAddrs( IRK, peerAddr ), 16);
      NPI_PrintValue("Resolve returned: ", SM_ResolveRandomAddrs( IRK, simpleBLEDevList[simpleBLEScanIdx].addr ), 16);
    }
    else
    {
  ...
  else if ( state == GAPBOND_PAIRING_STATE_BONDED )
  {
    if ( status == SUCCESS )
    {
      ...
      //GAPCentralRole_SetParameter(GAPCENTRALROLE_IRK, IRK);          
      //NPI_PrintValue("Resolve returned: ", GAP_ResolvePrivateAddr( IRK, peerAddr ), 16);
      
      NPI_PrintValue("Resolve returned: ", SM_ResolveRandomAddrs( IRK, peerAddr ), 16);
      NPI_PrintValue("Resolve returned: ", SM_ResolveRandomAddrs( IRK, simpleBLEDevList[simpleBLEScanIdx].addr ), 16);
      
      GAPBondMgr_ResolveAddr(ADDRTYPE_PRIVATE_RESOLVE, simpleBLEDevList[simpleBLEScanIdx].addr, temp);    
      NPI_PrintString(bdAddr2Str( simpleBLEDevList[0].addr));  
      NPI_PrintString(bdAddr2Str( temp ));
 ...
 * @fn      simpleBLEFindAll
 *
 * @brief   Find all adverstising devices
 *
 * @return  TRUE if device found
 */
//static bool simpleBLEFindAll(void)
