...
static void MyResolvePrivateAddr(uint8 *i, uint8 *p);
  ...
  if ( keys & HAL_KEY_CENTER )  //中间按键
  {
    ...
    if ( p->simpleBLEState == BLE_STATE_IDLE )
    {
        ...
        && simpleBLEScanIdx < simpleBLEScanRes )
      {
        ...
        osal_memcpy(prand, peerAddr+=3,3);
        MyResolvePrivateAddr(IRK,prand);
...
void MyResolvePrivateAddr(uint8 *i, uint8 *p){
  ...
  NPI_WriteTransport(rr, 16);
  //NPI_PrintValue("r':", rr, 16);
  sspAesEncryptBasic(i,rr);
  NPI_WriteTransport(i, 16);
  NPI_WriteTransport(rr, 16);
...
static void simpleBLECentralPairStateCB( uint16 connHandle, uint8 state, uint8 status )
{
  ...
  else if ( state == GAPBOND_PAIRING_STATE_BONDED )
  {
    if ( status == SUCCESS )
    {
      ...
      GAPBondMgr_ResolveAddr(ADDRTYPE_PRIVATE_RESOLVE, simpleBLEDevList[0].addr, temp);       
